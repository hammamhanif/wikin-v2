...
namespace App\Livewire\Pemas;

use App\Models\pemas;
use Livewire\Component;
use App\Models\LikesPemas;
use Illuminate\Support\Facades\Auth;
use App\Models\CommentPemas as ModelsComment;

class Comment extends Component
{
    public $body, $body2, $pemas;
    public $comment_id, $edit_comment_id;

    public function mount($id)
    {
        $this->pemas = pemas::find($id);
    }
    public function render()
    {
        return view('livewire.pemas.comment', [
            'comments' => ModelsComment::with(['user', 'childrens'])
                ->where('pemas_id', $this->pemas->id)
                ->whereNull('comment_id')->get(),
            'total_comments' => ModelsComment::where('pemas_id', $this->pemas->id)->count(),
        ]);
    }
    ...
    public function store()
    {
        ...
        if ($comment) {
            ...
            return redirect()->route('detail', $this->pemas->slug);
        }
    }
    ...
    public function like($id)
    {
        ...
        $like = LikesPemas::where($data);
        if ($like->count() > 0) {
            $like->delete();
        } else {
            LikesPemas::create($data);
        }
        return NULL;
    }
}
